...
对应工程目录：`machine-apps/`（machine-iam-app、machine-admin-app、machine-partner-app、machine-openapi-app、machine-mq-app、machine-xxljob-app）。
...
参考现有实现（如 `machine-iam-app` 中 `IamUserController`、`IamSupplierUserController`，`machine-partner-app` 中 `SuperOrganizationController`，`machine-openapi-app` 中 `OpenApiShopController` 等）：
