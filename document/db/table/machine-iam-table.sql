...
DROP TABLE IF EXISTS `t_user_type_relation`;
CREATE TABLE `t_user_type_relation` (
    `id` VARCHAR ( 32 ) NOT NULL COMMENT 'ID',
...
CREATE TABLE `t_permission` (
	...
	`resource_type` VARCHAR ( 8 ) NOT NULL COMMENT '资源类型，APP、MODULE、MENU、BUTTON',
	`code` VARCHAR ( 128 ) NOT NULL COMMENT '编码',
	`name` VARCHAR ( 32 ) NOT NULL COMMENT '名称',
	`path` VARCHAR ( 128 ) NOT NULL DEFAULT '' COMMENT '路径',
	`icon_url` VARCHAR ( 256 ) NOT NULL DEFAULT '' COMMENT '图标',
...
CREATE TABLE `t_role_permission_relation` (
	...
	`data_into` VARCHAR ( 2048 ) NOT NULL DEFAULT '' COMMENT '数据权限',
	`sort` BIGINT UNSIGNED NOT NULL default 0 COMMENT '排序',
	`create_by` VARCHAR ( 32 ) NOT NULL COMMENT '创建人',
	`create_time` BIGINT UNSIGNED NOT NULL COMMENT '创建时间',
	`update_by` VARCHAR ( 32 ) NOT NULL COMMENT '修改人',
...
DROP TABLE IF EXISTS `t_organization_user_relation`;
CREATE TABLE `t_organization_user_relation` (
    `id` VARCHAR ( 32 ) NOT NULL COMMENT 'ID',
...
DROP TABLE IF EXISTS `t_third_party_user`;
CREATE TABLE `t_third_party_user` (
	`id` VARCHAR ( 32 ) NOT NULL COMMENT 'ID',
	`source` VARCHAR ( 16 ) NOT NULL COMMENT '来源',
	`open_id` VARCHAR ( 32 ) NOT NULL COMMENT '用户openID',
	`union_id` VARCHAR ( 32 ) NOT NULL COMMENT 'UnionId',
	...
	`update_time` BIGINT UNSIGNED NOT NULL COMMENT '更新时间',
	PRIMARY KEY ( `id` ) USING BTREE,
	UNIQUE KEY `uk_01` ( `open_id`,`source` ) USING BTREE,
	KEY `idx_01` ( `union_id` ) USING BTREE,
    KEY `idx_02` ( `user_name` ) USING BTREE
) COMMENT = '第三方用户表';

DROP TABLE IF EXISTS `t_user_third_party_user_relation`;
CREATE TABLE `t_user_third_party_user_relation` (
    `id` VARCHAR ( 32 ) NOT NULL COMMENT 'ID',
    `user_id` VARCHAR ( 32 ) NOT NULL COMMENT '用户ID',
	...
	`update_time` BIGINT UNSIGNED NOT NULL COMMENT '更新时间',
	PRIMARY KEY ( `id` ) USING BTREE,
    UNIQUE KEY `uk_01` ( `user_id`,`third_party_user_id` ) USING BTREE,
    KEY `idx_01` ( `third_party_user_id` ) USING BTREE
) COMMENT = '用户&第三方用户关系表';

DROP TABLE IF EXISTS `t_user_role_target_relation`;
CREATE TABLE `t_user_role_target_relation` (
    `id` VARCHAR ( 32 ) NOT NULL COMMENT 'ID',
    `user_id` VARCHAR ( 32 ) NOT NULL COMMENT '用户ID',
	`role_id` VARCHAR ( 32 ) NOT NULL COMMENT '角色id',
	`target_id` VARCHAR ( 32 ) NOT NULL DEFAULT '' COMMENT '目标id',
	`target_type` VARCHAR ( 16 )  COMMENT '类型（UserRoleTargetTypeEnum）',
	`sort` BIGINT UNSIGNED NOT NULL default 0 COMMENT '排序',
	`create_by` VARCHAR ( 32 ) NOT NULL COMMENT '创建人',
	`create_time` BIGINT UNSIGNED NOT NULL COMMENT '创建时间',
	`update_by` VARCHAR ( 32 ) NOT NULL COMMENT '修改人',
	`update_time` BIGINT UNSIGNED NOT NULL COMMENT '更新时间',
	PRIMARY KEY ( `id` ) USING BTREE,
    UNIQUE KEY `uk_01` ( `user_id`,`target_id`,`role_id`,`target_type`) USING BTREE,
	KEY `idx_01` (`user_id`,`role_id`,`target_id` ) USING BTREE,
	KEY `idx_02` ( `target_id`,`target_type` ) USING BTREE
) COMMENT = '用户角色目标关系表';
...
CREATE TABLE `t_user_login_log` (
	...
	KEY `idx_03` ( `user_name` ) USING BTREE,
...
DROP TABLE IF EXISTS `t_oauth2_authorization_consent`;
CREATE TABLE `t_oauth2_authorization_consent` (
...
DROP TABLE IF EXISTS `t_oauth2_authorization`;
CREATE TABLE `t_oauth2_authorization` (
	`id` VARCHAR ( 128 ) NOT NULL COMMENT '授权记录ID',
	`registered_client_id` VARCHAR ( 128 ) NOT NULL COMMENT '注册客户端ID',
	`principal_name` VARCHAR ( 256 ) NOT NULL COMMENT '主体名称',
