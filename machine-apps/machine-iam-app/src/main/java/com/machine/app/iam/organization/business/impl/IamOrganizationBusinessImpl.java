...
import static com.machine.sdk.common.constant.CommonConstant.Data.ORGANIZATION_VIRTUAL_NODE_NAME;
...
public class IamOrganizationBusinessImpl implements IIamOrganizationBusiness {
    ...
    @Autowired
    private IIamOrganizationUserRelationClient organizationUserRelationClient;
    ...
    @Override
    public IamOrganizationDetailResponseVo detailByName(IamOrganizationDetailByNameInputDto inputDto) {
        IamOrganizationDetailOutputDto outputDto = organizationClient.detailByName(inputDto);
        if (null == outputDto) {
            return null;
        }
        return getDetailResponse(outputDto);
    ...
    @Override
    public IamOrganizationTreeSimpleOutputDto treeAllSimple(IamOrganizationQueryTreeRequestVo request) {
        return organizationCache.treeAllSimple(request.getType());
    }

    @Override
    public IamOrganizationExpandTreeResponseVo treeAllExpand(IamOrganizationQueryTreeRequestVo request) {
        ...
        //查询组织关联的门店信息
        List<ShopOrganizationRelationListOutputDto> shopOrganizationRelationListOutputDtoList = shopOrganizationRelationClient
        ...
        {//门店数量
            for (ShopOrganizationRelationListOutputDto dto : shopOrganizationRelationListOutputDtoList) {
        ...
        { //部门负责人信息
            Map<String, String> organizationIdUserIdMap = organizationUserRelationClient.mapByOrganizationIdSet(new IdSetRequest(organizationIdSet));
            if (CollectionUtil.isNotEmpty(organizationIdUserIdMap)) {
                Set<String> leaderIdSet = new HashSet<>(organizationIdUserIdMap.values());
                Map<String, UserDetailOutputDto> leaderDetailMap = userClient.mapByIdSet(new IdSetRequest(leaderIdSet));
                for (IamOrganizationExpandTreeBo bo : expandTreeBoList) {
                    String leaderId = organizationIdUserIdMap.get(bo.getId());
                    if (StrUtil.isNotEmpty(leaderId)) {
                        UserDetailOutputDto leaderDetail = leaderDetailMap.get(leaderId);
                        bo.setOrganizationLeader(JSONUtil.toBean(JSONUtil.toJsonStr(leaderDetail), IamOrganizationLeaderResponseVo.class));
                    }
                }
            }
        }
        ...
        { //添加【未分配】
            ...
            //查询未绑定组织的用户数量
            int userNumber = userClient.countNotBindOrganization(new DataUserNotBindOrganizationInputDto(request.getType()));
    ...
    @Override
    public IamOrganizationExpandWithShopTreeResponseVo treeAllExpandWithShop(IamOrganizationQueryTreeRequestVo request) {
        String typeName = request.getType().getName();
        List<IamOrganizationListOutputDto> outputDtoList = organizationClient.listAllByType(request.getType());
        Set<String> organizationIdSet = outputDtoList.stream().map(IamOrganizationListOutputDto::getId).collect(Collectors.toSet());
        ...
        //查询所有门店信息
        List<DataShopListOutputDto> shopListOutputDtoList = shopClient.listAll(new DataShopQueryListAllInputDto());
        Map<String, DataShopListOutputDto> shopOutputDtoMap = shopListOutputDtoList.stream()
                .collect(Collectors.toMap(DataShopListOutputDto::getId, dto -> dto));
        ...
        {//门店数量
            ...
            for (IamOrganizationExpandTreeBo expandTreeBo : expandTreeBoList) {
                ...
                if (CollectionUtil.isNotEmpty(shopIdSet)) {
                    List<DataShopListOutputDto> bindShopList = new ArrayList<>(shopIdSet.size());
        ...
        return JSONUtil.toBean(JSONUtil.toJsonStr(expandTreeBo), IamOrganizationExpandWithShopTreeResponseVo.class);
    }
    ...
    private IamOrganizationDetailResponseVo getDetailResponse(IamOrganizationDetailOutputDto outputDto) {
        ...
        //部门负责人信息
        IamOrganizationUserRelationOutputDto relationOutputDto =
                organizationUserRelationClient.selectByOrganizationId(new IdRequest(organizationId));
        if (null != relationOutputDto) {
            ...
            responseVo.setOrganizationLeader(JSONUtil.toBean(
                    JSONUtil.toJsonStr(userDetailOutputDto), IamOrganizationLeaderResponseVo.class));
        }
        ...
        if (CollectionUtil.isEmpty(shopOrganizationRelationListOutputDtoList)) {
            ...
            Set<String> shopIdSet = shopOrganizationRelationListOutputDtoList.stream().map(ShopOrganizationRelationListOutputDto::getShopId).collect(Collectors.toSet());
