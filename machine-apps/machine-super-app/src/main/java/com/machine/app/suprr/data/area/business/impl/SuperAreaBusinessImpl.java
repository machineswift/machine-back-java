...
public class SuperAreaBusinessImpl implements ISuperAreaBusiness {
    ...
    @Override
    public Set<String> recursionListSubIds(Set<String> areaIdSet) {
        if (CollectionUtil.isEmpty(areaIdSet)) {
            return Set.of();
        }

        //获取对应的节点
        AreaTreeOutputDto allOutputDto = treeAllSimple(new SuperAreaTreeRequestVo());
        List<AreaTreeOutputDto> treeOutputDtoList = new ArrayList<>();
        for (String areaId : areaIdSet) {
            AreaTreeOutputDto node = TreeUtil.findNode(allOutputDto, areaId);
            if (null != node) {
        ...
        //循环递归获取所有节点
        Set<String> reaIdSet = new HashSet<>();
        for (AreaTreeOutputDto treeOutputDto : treeOutputDtoList) {
            List<AreaTreeOutputDto> outputDtoList = TreeUtil.collectAllNodes(treeOutputDto);
            reaIdSet.addAll(outputDtoList.stream().map(AreaTreeOutputDto::getId).toList());
        }
        return reaIdSet;
    }
    ...
    public SuperAreaTreeExpandSelfResponseVo treeSelfExpand(SuperAreaTreeRequestVo request) {
        ...
        AreaTreeOutputDto filterTreeOutput = TreeUtil.filterTree(allTreeOutput, areaCodeSet);
    ...
    @Override
    public AreaTreeOutputDto treeAllSimple(SuperAreaTreeRequestVo request) {
