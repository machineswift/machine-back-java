...
public class SuperOrganizationBusinessImpl implements ISuperOrganizationBusiness {
    ...
    public SuperOrganizationTreeSimpleSelfResponseVo treeSelfSimple(SupeOrganizationTreeRequestVo request) {
        ...
        List<ShopOrganizationRelationListOutputDto> relationListOutputDtoList = shopOrganizationRelationClient.listByCondition(queryListInputDto);

        //获取拥有权限的树
        IamOrganizationTreeSimpleOutputDto filterTreeOutput = TreeUtil.filterTree(allTreeOutput,
                relationListOutputDtoList.stream().map(ShopOrganizationRelationListOutputDto::getOrganizationId).collect(Collectors.toSet()));
        ...
        if (CollectionUtil.isNotEmpty(relationListOutputDtoList)) {
            organizationIdShopIdSetmap = relationListOutputDtoList.stream()
                    .collect(Collectors.groupingBy(
                            ShopOrganizationRelationListOutputDto::getOrganizationId,
                            Collectors.mapping(ShopOrganizationRelationListOutputDto::getShopId,
                                    Collectors.toSet())));
        }
