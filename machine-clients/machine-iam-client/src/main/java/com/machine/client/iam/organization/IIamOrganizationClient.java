...
public interface IIamOrganizationClient {
    ...
    IamOrganizationDetailOutputDto detailByName(@RequestBody @Validated IamOrganizationDetailByNameInputDto inputDto);

    @GetMapping("list_all_by_type")
    List<IamOrganizationListOutputDto> listAllByType(@RequestParam("organizationType") OrganizationTypeEnum organizationType);

    /**
     * 区域树
     */
    @GetMapping("tree_all_simple")
    IamOrganizationTreeSimpleOutputDto treeAllSimple(@RequestParam("organizationType") OrganizationTypeEnum organizationType);
