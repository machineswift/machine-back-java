...
import com.machine.client.data.filecenter.attachment.IDataFileClient;
...
@Slf4j
@RestController
@RequestMapping("server/data/file_center/file")
public class DataFileServer implements IDataFileClient {
    ...
    public DataFileDetailOutputDto getById(@RequestBody @Validated IdRequest request) {
        return fileService.getById(request.getId());
    }

    @Override
    @PostMapping("map_by_idSet")
    public Map<String, DataFileDetailOutputDto> mapByIdSet(@RequestBody @Validated IdSetRequest request) {
        List<DataFileDetailOutputDto> outputDtoList = fileService.listByIdSet(request.getIdSet());
