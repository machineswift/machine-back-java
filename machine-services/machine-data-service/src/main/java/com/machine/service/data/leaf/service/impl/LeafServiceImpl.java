...
@Slf4j
@Service
public class LeafServiceImpl implements ILeafService {

    @Autowired
    private ILeafDao leafDao;
    ...
    public Long getLeafId(String bizTag,
        ...
        if (id == null) {
            ...
            LeafEntity leafAlloc = leafDao.updateMaxId(entity);
            if (leafAlloc == null) {
                throw new DataBusinessException("data.leaf", "leaf 取号失败");
            }
    ...
    private Long allocateLeafId(LeafData leafData, LeafEntity leafAlloc) {
        Long id;
        if (leafAlloc.getStep() > 1) {
            ...
            throw new DataBusinessException("data.leaf", "获取leaf数据失败,step 小于1");
        }
        return id;
    }
    ...
    private void loadTenantInfo() {
        ...
        leafMap.entrySet().removeIf(entry -> {
            ...
            return leafData != null && leafData.getExpireTime() != null &&
