...
import com.machine.service.data.mesage.service.IMqReliableMessageService;
...
@Slf4j
@Service
public class MqReliableMessageServiceImpl implements IMqReliableMessageService {
    ...
    @Autowired
    private IMqDeadMessageDao mqDeadMessageDao;

    @Autowired
    private IMqReliableMessageDao reliableMessageDao;

    @Autowired
    private IMqMessageExternalDao mqMessageExternalDao;


    @Override
    public void create(MqReliableMessageCreateInputDto inputDto) {
        MqReliableMessageEntity insertEntity = JSONUtil.toBean(JSONUtil.toJsonStr(inputDto), MqReliableMessageEntity.class);
        String reliableMessageId = reliableMessageDao.create(insertEntity);

        MqMessageExternalEntity insertExternalEntity = new MqMessageExternalEntity();
    ...
    public void resendMessage() {
        ...
        while (true) {
            List<MqReliableMessageEntity> messageReliableList = reliableMessageDao.dynamicSelect(unixTimestamp);

            if (CollectionUtils.isEmpty(messageReliableList)) {
                return;
            }

            for (MqReliableMessageEntity reliableMessage : messageReliableList) {
                ...
                if (count > 0) {
                    //查询内容
                    MqMessageExternalEntity externalEntity = mqMessageExternalDao.getByReliableMessageId(reliableMessageId);
    ...
    public MqReliableMessageDetailDto getById(String id) {
        MqReliableMessageEntity messageEntity = reliableMessageDao.getById(id);
