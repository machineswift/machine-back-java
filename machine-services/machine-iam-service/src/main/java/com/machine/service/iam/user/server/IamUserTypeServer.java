...
public class IamUserTypeServer implements IIamUserTypeClient {
    ...
    public List<UserTypeEnum> listTypeByUserId(@RequestBody @Validated IdRequest request) {
        return userTypeService.listTypeByUserId(request);
    }

    @Override
    @PostMapping("map_type_by_userIdSet")
    public Map<String, List<UserTypeEnum>> mapTypeByUserIdSet(@RequestBody @Validated IdSetRequest request) {
