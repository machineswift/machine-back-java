...
@Slf4j
@Component
public class RedisIamDataPermissionCache {
    ...
    public DataPermissionDto dataPermission4manage(String functionCode) {
        ...
        String permissionCode = appContextHolder.getPermissionCode();
        if (StrUtil.isBlank(permissionCode)) {
            throw new IamPermissionBusinessException("iam.permission.data.nullPermissionCode", "权限编码为空");
        }

        //获取数据权限编码
        String dataPermissionCode = appContextHolder.getDataPermissionCode();
        if (StrUtil.isBlank(dataPermissionCode)) {
            ...
            if (IamPermissionResourceTypeEnum.MENU == permissionDetailOutputDto.getResourceType()) {
                appContextHolder.setDataPermissionCode(permissionCode);
                dataPermissionCode = permissionCode;
            } else {
                IamPermissionTreeOutputDto rootTreeOutputDto = redisIamPermissionCache.treeAll();
                ...
                if (targetNode != null) {
                    appContextHolder.setDataPermissionCode(targetNode.getCode());
