...
import com.machine.starter.security.login.auth2.feishu.FeiShuAuthenticationFilter;
import com.machine.starter.security.login.auth2.feishu.FeiShuAuthenticationProvider;
import com.machine.starter.security.login.auth2.gitee.GiteeAuthenticationFilter;
import com.machine.starter.security.login.auth2.gitee.GiteeAuthenticationProvider;
...
public class CustomerWebSecurityConfig {
    ...
    public SecurityFilterChain loginFilterChain(HttpSecurity http) throws Exception {
        ...
        UserNameAuthenticationFilter usernameLoginFilter = new UserNameAuthenticationFilter(
                new AntPathRequestMatcher("/iam/auth/login/username", HttpMethod.POST.name()),
        ...
        SmsAuthenticationFilter smsLoginFilter = new SmsAuthenticationFilter(
                new AntPathRequestMatcher("/iam/auth/login/phone_captcha", HttpMethod.POST.name()),
        ...
        GiteeAuthenticationFilter giteeLoginFilter = new GiteeAuthenticationFilter(
                new AntPathRequestMatcher("/iam/auth2/login/gitee", HttpMethod.GET.name()),
        ...
        //登录方式:auth2.0-feishu
        FeiShuAuthenticationFilter feishuLoginFilter = new FeiShuAuthenticationFilter(
                new AntPathRequestMatcher("/iam/auth2/login/fei_shu", HttpMethod.GET.name()),
                new ProviderManager(List.of(applicationContext.getBean(FeiShuAuthenticationProvider.class))),
                customerLoginSuccessHandler,
                customerLoginFailureHandler);
        http.addFilterBefore(feishuLoginFilter, UsernamePasswordAuthenticationFilter.class);
