...
public class CustomerLoginFailureHandler implements AuthenticationFailureHandler {
    ...
    public void onAuthenticationFailure(HttpServletRequest request,
        ...
        if (e instanceof CaptchaWrongAuthenticationException) {
            ...
            result = AppResult.fail(HttpStatus.UNAUTHORIZED.value(), "iam.auth.authentication.captchaWrong", e.getMessage());
        } else if (e instanceof AuthFeignUserIdException) {
            response.setStatus(HttpStatus.UNAUTHORIZED.value());
            result = AppResult.fail(HttpStatus.UNAUTHORIZED.value(), "iam.auth.authentication.feignApplyUserIdLost", e.getMessage());
        }else if (e instanceof AuthInterceptorUserIdException) {
            response.setStatus(HttpStatus.UNAUTHORIZED.value());
            result = AppResult.fail(HttpStatus.UNAUTHORIZED.value(), "iam.auth.authentication.interceptorPreHandleUserIdLost", e.getMessage());
        }  else if (e instanceof UserStatusDisableException) {
            response.setStatus(HttpStatus.UNAUTHORIZED.value());
            result = AppResult.fail(HttpStatus.UNAUTHORIZED.value(), "iam.auth.authentication.userStatusDisable",  e.getMessage());
        } else if (e instanceof UsernameNotFoundException) {
            response.setStatus(HttpStatus.UNAUTHORIZED.value());
            result = AppResult.fail(HttpStatus.UNAUTHORIZED.value(), "iam.auth.authentication.usernameOrPasswordWrong", "用户名或密码不正确");
        }  else if (e instanceof PhoneNotFoundException) {
            response.setStatus(HttpStatus.UNAUTHORIZED.value());
            result = AppResult.fail(HttpStatus.UNAUTHORIZED.value(), "iam.auth.authentication.phoneNotFound",
                    "您的手机号当前无权限登录，请检查账号是否正确或联系客服");
        } else {
            response.setStatus(HttpStatus.UNAUTHORIZED.value());
            result = AppResult.fail(HttpStatus.UNAUTHORIZED.value(), "iam.auth.authentication", e.getMessage());
        }
        ...
        if (null != AppContext.getContext().getUserId()) {
            ...
            inputDto.setAuthAction(AuthActionEnum.LOGIN);
            inputDto.setAuthMethod(AppContext.getContext().getAuthMethod());
            inputDto.setAuthResult(AuthResultEnum.FAIL);
